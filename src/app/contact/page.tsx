    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <section className="relative h-56 overflow-hidden md:h-72">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center px-4 text-center">
          <Badge className="mb-3 rounded-full border border-zinc-600 bg-black/50 px-4 py-1 text-zinc-100">
            Contact KNWN
          </Badge>
          <h1 className="font-display text-5xl font-extrabold uppercase leading-none tracking-tight text-white md:text-7xl">
            Claim Your Spot.
          </h1>
          <p className="mt-3 max-w-lg text-sm text-zinc-300 md:text-base">
      ...
      <main className="mx-auto max-w-2xl px-4 py-10 md:py-14">
        <Card className="border-zinc-800 bg-zinc-900/70 p-6 md:p-8">
        ...
        <section id="faq" className="mt-10">
          ...
          <Card className="border-zinc-800 bg-zinc-900/70 p-2">
            <Accordion type="single" collapsible className="w-full">
              {FAQS.map((faq, idx) => (
                <AccordionItem key={faq.q} value={`faq-${idx}`} className="border-zinc-800 px-4">
                  <AccordionTrigger className="text-left text-sm font-medium text-white">
                    {faq.q}
                  </AccordionTrigger>
                  <AccordionContent className="text-sm text-zinc-300">{faq.a}</AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
          </Card>
